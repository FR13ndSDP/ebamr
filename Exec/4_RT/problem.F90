...
subroutine initdata_f(level, time, lo, hi, u, ulo, uhi, dx, prob_lo, prob_hi) bind(C, name="initdata_f")
  ...
  integer :: i,j,k
  real(rt) :: x, y, z, Pt, rhot, uxt, vxt, pi = 3.14159265358979d0
  real(rt) :: splitx, splity, splitz, L_x, presmid, r2d, pertheight

  splitx = 0.5d0*(prob_lo(1)+prob_hi(1))
  splity = 0.5d0*(prob_lo(2)+prob_hi(2))
  splitz = 0.5d0*(prob_lo(3)+prob_hi(3))
  L_x = prob_hi(1) - prob_lo(1)
  presmid = p_base - rho_r * splitz
  ...
  do k = lo(3), hi(3)
    ...
    do j = lo(2), hi(2)
      ...
      do i = lo(1), hi(1)
        ...
        r2d = min(0.5*L_x, hypot(x-splitx,y-splity))
        pertheight = 0.5d0 - 0.01d0*cos(2.d0*pi*r2d/L_x)
        rhot = rho_r + (rho_l-rho_r)/2.d0 * (1.d0+tanh((z-pertheight)/0.005d0))
        u(i,j,k,urho) = rhot
        u(i,j,k,umx:umz) = 0.d0
        u(i,j,k,ueden) = Pt/(gamma - 1.d0)
      end do
    end do
  end do
